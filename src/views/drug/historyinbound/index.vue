<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="入库数量（最小销售包装单位）" prop="inboundPackQuantity">
        <el-input
          v-model="queryParams.inboundPackQuantity"
      ...
      <el-form-item label="入库数量（最小制剂单位）" prop="inboundDosageQuantity">
        <el-input
          v-model="queryParams.inboundDosageQuantity"
      ...
      <el-form-item label="入库总金额（元）" prop="inboundTotalAmount">
        <el-input
          v-model="queryParams.inboundTotalAmount"
          placeholder="请输入入库总金额（元）"
          clearable
          @keyup.enter="handleQuery"
          class="!w-240px"
        />
      ...
      <el-form-item label="入库价格（最小销售包装单位）" prop="inboundPackPrice">
        <el-input
          v-model="queryParams.inboundPackPrice"
      ...
      <el-form-item label="入库价格（最小制剂单位）" prop="inboundDosagePrice">
        <el-input
          v-model="queryParams.inboundDosagePrice"
      ...
      <el-form-item label="价格状态:0-正常,1-价格过高,2-价格过低" prop="priceStatus">
        <el-select
          ...
          placeholder="请选择价格状态:0-正常,1-价格过高,2-价格过低"
          clearable
          class="!w-240px"
        >
          <el-option label="请选择字典生成" value="" />
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['drug:history-inbound:create']"
        >
        ...
        <el-button
          ...
          v-hasPermi="['drug:history-inbound:export']"
        >
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="入库数量（最小销售包装单位）" align="center" prop="inboundPackQuantity" />
      <el-table-column label="入库数量（最小制剂单位）" align="center" prop="inboundDosageQuantity" />
      <el-table-column label="入库总金额（元）" align="center" prop="inboundTotalAmount" />
      <el-table-column label="入库价格（最小销售包装单位）" align="center" prop="inboundPackPrice" />
      <el-table-column label="入库价格（最小制剂单位）" align="center" prop="inboundDosagePrice" />
      ...
      <el-table-column label="价格状态:0-正常,1-价格过高,2-价格过低" align="center" prop="priceStatus" />
      <el-table-column
        label="创建时间"
        align="center"
        prop="createTime"
      ...
      <el-table-column label="操作" align="center" min-width="120px">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['drug:history-inbound:update']"
          >
            编辑
          </el-button>
          <el-button
            ...
            v-hasPermi="['drug:history-inbound:delete']"
          >
            删除
          </el-button>
        </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <HistoryInboundForm ref="formRef" @success="getList" />
</template>

<script setup lang="ts">
import { dateFormatter } from '@/utils/formatTime'
import download from '@/utils/download'
import { HistoryInboundApi, HistoryInboundVO } from '@/api/drug/historyinbound'
import HistoryInboundForm from './HistoryInboundForm.vue'

/** 药品入库情况-历史 列表 */
defineOptions({ name: 'HistoryInbound' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<HistoryInboundVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await HistoryInboundApi.getHistoryInboundPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await HistoryInboundApi.deleteHistoryInbound(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await HistoryInboundApi.exportHistoryInbound(queryParams)
    download.excel(data, '药品入库情况-历史.xls')
  } catch {
  } finally {
